...
async def get_regions():
    ...
    for name, region in glb.regions.items():
        ...
        region_dict["status"] = region.status.value
        # Convert datetime to ISO format string
        if region_dict["last_health_check"]:
            region_dict["last_health_check"] = region_dict["last_health_check"].isoformat()
        result[name] = {
            **region_dict,
            "circuit_breaker": {
...
async def get_metrics():
    ...
    healthy_regions = sum(1 for r in glb.regions.values() if r.status.value == "healthy")
    ...
    try:
        while True:
            try:
                ...
                # Apply enum conversion to the entire data structure
                converted_data = convert_enums(data)
                await websocket.send_json(converted_data)
