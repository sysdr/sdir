"""
Web Dashboard for Checkpoint and Rollback Recovery Demo
...
# Import models from task_processor
import sys
sys.path.append('/app/src')
from task_processor import Base, Task, Checkpoint
...
async def startup():
    global db_session, redis_client
...
async def create_checkpoint():
    ...
    try:
        # Signal task processor to create checkpoint
        await redis_client.publish('control', json.dumps({
...
async def rollback_to_checkpoint(checkpoint_id: str):
    ...
    try:
        # Signal task processor to rollback
        await redis_client.publish('control', json.dumps({
            'action': 'rollback',
...
if __name__ == "__main__":
    # Create web templates directory and files
    ...
    <div class="container">
        <div class="header">
            ...
            <div style="margin-top: 12px;">
                ...
                <span class="timestamp" style="margin-left: 16px;" id="last-update">Last update: --</span>
            </div>
        </div>
            ...
            <div class="card">
                ...
                <div class="status-grid" id="task-stats">
                    ...
                    <div class="status-item status-processing">
                        <div style="font-size: 24px; font-weight: bold;" id="processing-count">0</div>
                        <div>Processing</div>
                    </div>
                    <div class="status-item status-completed">
                        <div style="font-size: 24px; font-weight: bold;" id="completed-count">0</div>
                        <div>Completed</div>
                    </div>
                    <div class="status-item status-failed">
                        <div style="font-size: 24px; font-weight: bold;" id="failed-count">0</div>
                        <div>Failed</div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function handleWebSocketMessage(message) {
            if (message.type === 'stats_update') {
                ...
                showNotification('Checkpoint creation triggered');
